...
the Velora scoping fields (storyline + owning entity) the store/indexer need.
"""
...
class EntryType(str, Enum):
    """Semantic kind of a lore entry (brief §1.3). Velora entities map onto these:
...
class Frontmatter(BaseModel):
    ...
    Velora has no era axis today — left ``None`` for every current entry.
    """
...
class LoreEntry:
    """An indexable unit: validated front matter + body + Velora scoping.

    ``entity_type`` is the Velora source kind (``storyline`` | ``character`` |
