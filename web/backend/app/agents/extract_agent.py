...
This is what makes world population build the author's *own* people: a markdown file
naming several characters becomes several extracted character briefs (one entity
each), instead of collapsing into one or vanishing into ``other``-bucket lore. A
single-subject doc yields one entity; a pure lore/history doc (no profile-worthy
subject) yields none — it still grounds the world elsewhere.

Nothing is persisted here; ``services.world_populate`` drafts and persists one entity
per returned subject, grounded in that subject's ``source`` paragraph. No graph, no
retrieval.

Restored from the retired ``build_agent`` world build (removed 2026-07-10): losing it
is what left population inventing a cast from the premise while the author's uploaded,
classified character files went unread.
"""
...
_EXTRACT_SYSTEM = (
    "You are Mytheca's entity-extraction assistant for an interactive-fiction world. "
