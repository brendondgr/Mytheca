...
| C-010 | The latency a player experiences in Mytheca is dominated by the inference endpoint's own throughput variance, not by the app's turn structure. | [EXP-2026-08-006](experiments/EXP-2026-08-006-turn-latency-overhaul/) | [overhaul.png](experiments/EXP-2026-08-006-turn-latency-overhaul/figures/overhaul.png) | **partial** | Control probe, one fixed prompt, temperature 0, same served model (`qwen38-27B-awq`), **identical 143-token output in 5 of 6 runs**: elapsed 1.4 / 1.8 / 1.9 / 14.6 / 27.6 / 90.4 s — a **66x spread on byte-identical work**. Independently corroborated on prefill: two prompts that took 1.94 s and 2.22 s the previous day took 30.1 s and 30.3 s with an identical cached-token count. **Downgraded 2026-08-20 before any of it was relied on:** the probe did **not** control for concurrent load on the same GPU, which is an ordinary and sufficient explanation for identical output at wildly different wall-clock. The recorded elapsed times are fact; the attribution to the endpoint *itself* is not established, and neither is "dominates". A controlled repeat is pending; on the first attempt the host was fully down (`502 ConnectError`, relay health `failed`), which shows it comes and goes but does not settle the cause — establishing it needs the app's own cost measured against a stable endpoint, which no run has yet had. It also makes EXP-2026-08-005's causal reading of its two ~300 s turns as *planner* stalls unsupported. |
## The meta-claim
