...
│   │       ├── services/   # Turn loop: turn_engine (POV loop + delta streaming; persists the diagnostic trace), assembler (Band-1 context), retrieval_gate, emission (thin-tag parse), validator (stat clamp + presence), presence (scene-presence fold from the event log), events_store (seq/persist + session list/history/close), session_export (JSON/Markdown record), turn_writer (cold-path consequences); event engine; stat_guidance; media_cleanup; Story Graph: type_registry, graph_writer, graph_reader
