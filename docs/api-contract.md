...
`guidance` is **not** persisted on the `user_turn` row — it shapes the turn it was sent with and
appears in the trace, but is not restored into the box on resume.
