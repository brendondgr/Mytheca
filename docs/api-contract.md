...
| Scenarios | `GET /storylines/{id}/scenarios`, `POST /storylines/{id}/scenarios`, `GET /scenarios/{id}`, `PATCH /scenarios/{id}`, `DELETE /scenarios/{id}` | The live situations; may add/override stats. Read/write shape includes `image` (nullable relative `/media/scenes/...` URL of the generated WebP scene art), `sceneArtPositive`, and `sceneArtNegative` (nullable prompt strings), plus two **per-scene play controls** set from the composer: `maxTurns` (hard ceiling on character replies per player message, ≥1, default **5** — the loop may still end earlier) and `suggestionsCount` (how many follow-up suggestions to offer at the end of a turn, 0–4, `0` disables, default **4**). |
...
| Play | `POST /play/{scenarioId}/turn` | **Implemented.** Submit a player turn; the response body **is** the NDJSON event stream (`application/x-ndjson`, one event per line). Body: `{ text, directedAt?, sessionId?, mode?, trace?, outcome?, guidance? }` (omit `sessionId` to start a session). The engine **interprets the line** (narrate / address / **puppet** a character / whole-group), then runs a **ReAct planner** that decides the next beat after each one — a character speaks/acts (in their own voice; a puppeted character *performs* the direction), the narrator sets context, or the turn ends. Speaker order is dynamic; the back-and-forth is bounded by the scenario's **`maxTurns`** (a hard ceiling on character replies — the loop ends there even if the planner would continue). A **cold scene open** with no directed character is **narrator-led**. `guidance` (sent when the player selects a follow-up suggestion) steers the turn **open-endedly** — it nudges the planner + characters toward that general direction while the AI produces original, unscripted dialogue (it does NOT dictate a script; `outcome` is the legacy play-out tag). At the end of the turn, up to **`suggestionsCount`** follow-up suggestions (0–4) are generated from the **most recent line** and emitted as `branch_choices`. Character replies are grounded in their **graph relationships** (direct + 2-hop). Pre-flight failures (unknown scenario → 404, empty text → 400, bad session → 404/400) return a normal error envelope before the 200 stream opens; a mid-stream failure is the terminal `{ "type": "error", "message": "…" }` frame. See Turn Stream below. |
...
"mode": "pov" | "narrator", "outcome": "…" | null, "guidance": "…" | null }` (omit `sessionId`
to open a new play session; the streamed events carry the resolved `sessionId`; `mode`
defaults to `pov`). `guidance` (a selected follow-up suggestion) steers the turn open-endedly
— a soft nudge threaded into the planner + character prompts, not a script; `outcome` is the
legacy branch-direction tag that opened with a progression narration. The character back-and-forth
is capped by the scenario's `maxTurns`. A cold scene open with no directed character opens narrator-first.
...
D11; count-driven — no longer gated on a rarely-set planner flag); stats inform which surface,
never gate them. Selecting one sends `guidance` on the next turn (open-ended steer, not a script). The player's input is persisted as a
