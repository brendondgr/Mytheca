...
| Character stats | `GET /characters/{id}/stats`, `PUT /characters/{id}/stats`, `POST /characters/{id}/stats/reset` | The **authored** starting values (`character_stats`) — not what a play-through is worth, which lives in `session_character_stats` (owner decision D-1). The `PUT` is an authoring write and moves the row's `baseline` with its `value`. **`POST …/reset`** puts a character back to the values they were written with: the only way back from `session_stats.carry_forward`, which overwrites the authored value in place at session close for any stat marked `carryOver`. A stat that has never carried has no captured baseline and resets to itself, so a reset is safe on a whole cast without knowing their history, and 404s only on an unknown character. |
| Characters | `GET /storylines/{id}/characters`, `POST /storylines/{id}/characters`, `GET /characters/{id}`, `PATCH /characters/{id}`, `DELETE /characters/{id}` | Belong to a storyline; each holds a stat block. Read/write shape: `id`, `name`, `role`, `color`, `mono` (derived), `traits`, `speech`, `goal`, `secret`, plus base-identity prose `appearance`, `background`, `personality` (all nullable), `portrait` (nullable relative `/media/...` URL of the generated WebP avatar), `portraitPositive` / `portraitNegative` (nullable ComfyUI prompt strings that produced the portrait — persisted so the author can tweak-and-re-render on re-edit), and `voiceSamples` (a list of `{ situation, sample, moment }` pairs — the character's voice & tone profile, each pair a previous situation paired with the character's *single* in-voice response to it (never a back-and-forth exchange); empty list when unauthored, derived from background/personality before starting stats and injected into the turn loop). **`moment`** tags which kind of moment the pair demonstrates — `"light"` \| `"neutral"` \| `"tense"` \| `"grave"`, or `""` for "any moment" (an unrecognized value is coerced to `""` rather than rejected). Only the pairs matching the beat's **register** (plus untagged ones) reach the turn prompt, so a character has a concrete exemplar of itself *not at rest*. **`looseness`** is the character's own word-choice bias, an integer in `[-2, +2]` with `null` meaning neutral (422 outside that range). It is a **bias on top of the beat's register, not a replacement for it**: it nudges the register's `top_p` by 0.03 per step, clamped to `[0.70, 0.98]`, and **moves nothing else — the frequency and presence penalty columns stay at 0.0 and this dial must never be extended to them** (EXP-2026-08-007 measured those penalties degrading the sentence structure of prose). One notch is at most one register row's worth; at ±2 the range does cross a row, which is recorded beside the constant. The step size is chosen, not measured. |
...
| Scenarios | `GET /storylines/{id}/scenarios`, `POST /storylines/{id}/scenarios`, `GET /scenarios/{id}`, `PATCH /scenarios/{id}`, `DELETE /scenarios/{id}` | The live situations; may add/override stats. Read/write shape includes `image` (nullable relative `/media/scenes/...` URL of the generated WebP scene art), `sceneArtPositive`, and `sceneArtNegative` (nullable prompt strings), plus the **per-scene play controls**: **`sceneMode`** (`"structured"` — the plan, registers and attributed beats, everything that shipped before free-text mode; or `"freetext"` — **one unbroken passage per turn**, nobody scheduled and nothing attributed, run by `services/freetext_turn` instead of `turn_engine`. `null` reads as `"structured"`, deliberately **unlike** `sceneFlow` below: a mode decides what a turn *is*, so a silent column must never move an existing scene onto a different engine. Under `freetext` the `plannerMode` and `sceneFlow` controls are not read at all), `sceneFlow` (`"voiced"` — one model call per speaker, each carrying that character's voice samples; or `"continuous"` — ONE call writes the whole planned turn, marking each change of speaker with `<speaker:N>`. `null` reads as `"voiced"`, which is what has always shipped; a model that emits no hand-off tokens falls back to the per-speaker path for that turn and traces `sceneFlow: "fallback"`), `plannerMode` (`"auto"` | `"plan"` | `"off"`; the legacy `"planner"` is accepted and reads as `"auto"`), and `suggestionsCount` (how many follow-up suggestions to offer at the end of a turn, 0–4, `0` disables, default **4**), **`contextPolicy`** (`"auto"` | `"fixed"`; `null` reads as auto) and `contextBeats` (5–100, default **14**). Under the default **auto** policy the transcript window fits itself to the model's real context budget each turn (`services/context_budget`) and `contextBeats` is **ignored**; under `fixed` the scene keeps its own depth. Asking the player for a beat count was asking a question only the app can answer, so the control is gone from the UI — what the scene actually reached is *reported* on the new `window` trace step (`windowBeats`, `windowSource` ∈ `detected|configured|fallback|fixed`, `droppedBeats`, `budgetTokens`), which also rides on the `assemble` step's payload. When `TURN_CONTEXT_COMPACTION` is on, beats that fall out of the window are folded into a rolling per-session summary and a **`compaction`** trace step reports it (`beatsFolded`, `throughSeq`, `summaryChars`); `SessionSummary` carries **`summaryThroughSeq`** so a reload knows where verbatim recall ends without a second request. The summary is cleared whenever history is rewritten at or below that seq (rewind, beat edit, beat re-roll), because a stale summary is worse than none. `contextBeats` still validates (an out-of-range value is a 422), because it remains a real setting under `fixed`. **`maxTurns` and `beatLength` are gone as controls** (2026-08-24): how many beats a message makes and how long a beat is are decided by the scene, not set. Both columns remain on the row but are **unread** — dropping them would break any other process still running against the same database — and a request still sending them is accepted and ignored rather than 422'd. See *Adaptive pacing* below. Also includes **`directionVerbs`** — the scene's own one-tap direction verbs, appended to the built-in bar's groups: a list of `{label, group, text}` where `label` is the chip, `group` is a strict enum (`pace`|`tone`|`event`|`exit`, anything else a **422**, since the bar draws by group and an unknown one would be a verb that silently never renders) and `text` is the phrasing written into the player's direction box for them to edit. Both strings are required and non-empty, and the list is capped at **8** — the bar is a glance-and-tap surface. Nullable in the DB, so a scenario written before the column reads as `[]`. Also includes `promptOverrides` (nullable JSON object `{registryKey: text}` — per-scenario writing-agent prompt overrides, the innermost layer of the four-layer resolution chain; coerced to `{}` when NULL on read; see Writing-Agent Prompt Overrides below) and `styleBlocks` (nullable JSON object `{blockId: text}` — per-scene **narrative style overrides**, composed as an appended delta on the world's guide; coerced to `{}` when NULL on read; see Narrative Style Guide below). |
...
| Story Graph | `GET /scenarios/{id}/graph` | **Implemented.** Loads the scenario's Story-Graph subgraph — its cast + setting, plus one hop out into the factions, secrets, events, promoted subjects, adjacent settings and off-scene ties they are attached to — read live from Neo4j (§7.2). Returns `{ available, scenarioId, anchorIds, nodes[], edges[] }`, where `anchorIds` is the cast + setting before the expansion; `available` is `false` with empty lists when the graph is disabled/unreachable (best-effort). See Story Graph Shapes below. |
...
| Where a beat came from | `GET /play/{scenarioId}/sessions/{sessionId}/beats/{eventId}/memory` | **Implemented.** The memories one beat was written with — the player-facing "where did this line come from?" behind the per-beat source control. `BeatMemoryResponse`: `eventId`, `memories[]` of `{id, characterId, characterName, gloss, quote?, quoteSpeakerName?, scenarioId, scenarioTitle, sessionId, turnSeq, eventId?, inThisSession, contradictedBy[]}`. Reads the memory ids the engine stamps on the beat's own `Event.data` when it streams, so it costs the turn path nothing and works on a reopened scene. `eventId` inside a memory is the beat to scroll to and is resolved from `(sessionId, turnSeq)` rather than stored — an edit or re-roll replaces a beat's text in place, so a stored id could point at a line that no longer says what the memory is about. `contradictedBy` is other characters' memories of the **same turn** whose gloss differs; it exists because a cast that remembers subjectively will produce characters who flatly disagree, and a player with no way to see that reasonably concludes the app lost track. A beat with nothing behind it returns an **empty list, not a 404** — "this line came from the scene itself" is a real and common answer. 404 on an unknown beat, or one belonging to another play-through. Deliberately player-facing: scores, fade multipliers and cue hits are engine internals and stay in the Turn Inspector. |
...
| Edit a beat | `PATCH /play/{scenarioId}/sessions/{sessionId}/beats/{eventId}` | **Implemented.** Body `{ text, expectedSeq? }`. Rewrites one beat's prose and returns the row as a `PersistedEvent`. Allowed on `narration`, `character_prose`, `character_dialogue`, `character_action`, `internal_thought` and `user_turn` — including **any** player line, not just the last. A beat with no prose of its own (`state_update`, `branch_choices`, `character_status_change`, `scene_image`) is refused **422** rather than silently accepted. Sets `data.editedByPlayer`, and **rebuilds the recent-turn buffer**, without which the cast would keep reading the old wording out of Redis while the player reads the new one. Non-destructive: what followed the beat stays — discarding it is what rewind is for. The diagnostic trace deliberately still records what the model originally produced. **The story player offers Edit only on the player's own lines** — a `player` beat, or a POV beat they authored in a character's name (`StoryPlayerView.isPlayerAuthored`). The endpoint's breadth is unchanged and deliberate (the export and the take machinery share the row), but rewriting the model's prose puts words in a character's mouth their own state was never conditioned on, and the record's answer to a bad line from the cast is **Re-roll**, which regenerates it in place and keeps the previous wording as a take. |
| Branch a play-through | `POST /play/{scenarioId}/sessions/{sessionId}/branch` | **Implemented.** 201. Body `{ atEventId, name?, expectedSeq? }`. Forks the play-through at the **end of the turn** containing `atEventId` into a new session with `parentSessionId` + `forkSeq` set, copying its events and traces (fresh ids, **same seqs**), its session stat values, and the **outstanding direction the parent owed at the fork point** (a fork that dropped it would make "branch here and try again" quietly different from carrying on). It deliberately does **not** copy the rolling summary — that is a derived record which would go stale the moment the branch diverged, with no seq to notice by. The source is untouched. `expectedSeq` is an optimistic precondition — a mismatch 409s. |
| Rewind a play-through | `POST /play/{scenarioId}/sessions/{sessionId}/rewind` | **Implemented.** Body `{ atEventId, keepSnapshot=true, expectedSeq? }`. Cuts at a **turn boundary**: the whole turn containing `atEventId` goes, with everything after it — the opening `user_turn` row included, because the player is about to rewrite that line. With `keepSnapshot` the pre-cut history is forked into `Before rewind · <hh:mm>` first, so undo is a row in the tray rather than soft-deletion. Re-derives session stats by replaying the surviving `state_update` rows, rebuilds the Redis buffer, prunes the cut turns' Neo4j `:Event` nodes, **clears the cast's per-character interior state** (the stance each character re-enters with, which the cut beats produced) and **drops the outstanding direction those turns raised** (rows whose `fromTurn` is above the cut; see `docs/data-flow.md`). Relationship *edges* the cut turns wrote are still **not** rolled back — measured and recorded in `docs/checklist.md`. Returns `{ session, cutSeq, removedEvents, removedTraces, snapshotSessionId, restoredTurn }` — `restoredTurn` being the deleted player line with its `guidance`, `pov` and `taggedDocIds`, which is what lets the client hand the player their own words back to edit. **Rewinding into the first turn empties the play-through** — the containing turn goes and there is nothing before it. That is the specified behaviour, not a failure, and it is recoverable from the `Before rewind · hh:mm` fork; the story player now says so in the confirmation rather than reporting the same beat count it uses to trim a tail. |
...
The scenario subgraph (`GET /scenarios/{id}/graph`) — cast + setting, plus one hop out into
what they are attached to, read live from Neo4j. `anchorIds` separates the two: everything
in it is in the room, everything else in `nodes` is context reached around it.

```json
{
  "available": true,
  "scenarioId": "sc_…",
  "anchorIds": ["saltworn", "maerin", "aldous", "wren", "doran"],
  "nodes": [
    ...
    { "id": "fac-court", "type": "Faction", "label": "The Drowned Court", "storyline": "embergate", "metadata": { "desc": "…" } },
    { "id": "sec-chapel-fire", "type": "Secret", "label": "Aldous set the chapel fire himself", "storyline": "embergate", "metadata": { "severity": 0.9, "truthValue": "true" } }
  ],
  "edges": [
    { "source": "maerin", "target": "saltworn", "type": "present_at", "metadata": { "weight": 1.0, "visibility": "public", "status": "active" } },
    { "source": "aldous", "target": "sec-chapel-fire", "type": "knows", "metadata": { "reason": "He has confessed it to nobody." } }
  ]
}
```
...
- `GET /options/scene-presets` — **returns `[]`.** The four named presets (*Fast banter*,
  *Slow burn*, *Cinematic*, *Interrogation*) were defined **entirely** in terms of `maxTurns`
  and `beatLength`; with both removed there was nothing left for a preset to name, so the
  catalogue is empty and the composer's picker — which already hid itself on an empty list —
  no longer renders. The route is kept so no client 404s. `scenePreset` remains a column and
...
| `scene_prose` | **A whole free-text turn** — one unbroken passage holding the room, rendered as a page of prose with no speaker card, portrait or name inside it. Emitted only under `sceneMode: "freetext"` | `text` (delta-streams), `done`. **No `characterId`, and that absence is the type**: the passage belongs to the scene rather than to anyone in it. Every continuation pass re-emits the **same id**, so a turn that goes back for more extends the passage already on screen rather than appending a second block |
...
**`tasks` (transport, free-text only).** `{ "type": "tasks", "sessionId", "tasks": [{ "n", "must", "who": [names], "state": "" | "yes" | "partial" | "no", "note" }], "pass", "complete" }`.
The checklist a free-text turn writes for itself before any prose exists, and the same list
again with its verdicts once the review has run (one more frame per continuation pass). A
**transport** frame for the same reason `plan` is: a checklist states what a turn *intends*,
which is not a thing that happened, so it is never persisted and never reaches the transcript
— it renders in the Director rail. Rows are keyed on `n`, so a later frame updates the list in
place rather than stacking a second copy. `who` is a **note about who is involved, never a
schedule** — nothing in the engine dispatches on it. Like `plan`, it carries `sessionId`,
because on a new session it can be the first frame to arrive.
...
"guidance": "…" | null, "taggedDocIds": ["cd_…"], "overrides": { … } | null }` (omit `sessionId`
...
**Per-turn overrides (`overrides`).** Scene settings applied to **this turn only**:

```json
{ "suggestionsCount": 0-4 | null,
...
row, and an omitted or empty envelope behaves **exactly** as today. Bounds mirror
`ScenarioUpdate`'s. **`maxTurns` and `beatLength` used to ride here and are gone** — pacing is
the scene's judgement now — and an envelope still carrying them is accepted and **ignored**
rather than rejected, which is the right outcome for a control that no longer exists. An
out-of-range value on a control that *does* still exist is a 422 at the boundary; a hand-edited scenario row is re-clamped by the resolver
(`services/turn_settings.resolve`).

Three properties are the contract:

- **Per-turn, never persisted.** Nothing here is written to the `Scenario` row. The next turn
  that sends no envelope runs on the scene's own settings again.
- **Persisted for audit only.** The non-null map is written to the `user_turn` row as
  `data.overrides` (an additive JSON key, absent when nothing was overridden) and repeated on
  ...
  happened.
**Hidden stats.** A `state_update` for a stat whose `StatDefinition.visibility` is `hidden`
is **persisted, traced and exported but never streamed** — the emitter writes the row and
declines to yield it, and `rehydrateFromHistory` skips it on reload so a refreshed scene
agrees with the live one. The `stat` **trace** step still carries it: the Inspector is a
diagnostic surface and shows everything, the transcript is the story and does not. Until
2026-08-22 `visibility` had no effect at play time and a hidden stat announced itself.
...
- **Withheld from the agents.** Like `taggedDocIds`, it is out of scope for the intent,
  ...
  cannot decide what happens, who acts, or where the scene goes.
...
low-effort structural call (`direction_agent.parse`); in Playwright mode the requirements ride on
...
**The plan (transport, never a story event).** A planned turn emits
`{ "type": "plan", "sessionId", "beats": [...], "awaitingApproval": bool }`. Each beat is
`{ action, actorId, actorName, addressingId, reason, register, stakes, status }` — `actorName`
is resolved server-side so a panel never has to join against the cast to draw a row.

It is **not** a story event and is **not persisted**: a plan states what a turn *intends*,
which may not happen, and persisting one would put something in the transcript no reader saw
and no rewind could account for. It carries no `seq` and no envelope `id`.

Under `planner: "plan"` it arrives with `awaitingApproval: true` and **the turn stops there** —
no prose, no suggestions, no reflection. The client sends the same `beats` back as
`TurnRequest.approvedPlan` on a `continuation: true` turn, and the engine runs them with **no
second planner call**. `sessionId` is load-bearing on that turn: it emits no story event, and
story events are the only other frames carrying one — without it a client that started a new
session would have nothing to send the approval on. An approved beat naming somebody no longer
present is dropped; if *every* beat is, the turn plans afresh and traces `planner: "stale"`
rather than passing a different turn off as the player's.

Under `planner: "auto"` the same frame rides along for the Inspector **only when `trace: true`**
— emitting it unconditionally would insert a frame ahead of the first story event on every
turn, changing the shape of the default stream every existing consumer indexes into.
...
A **style guide** says *how this story is written*, where the World Primer says what is true
in it. Six optional blocks, stored as `styleBlocks` (`{blockId: text}`) on the storyline and,
as per-scene overrides, on the scenario.

| block | governs | who reads it | where it lands in the prompt |
| --- | --- | --- | --- |
...
| `signature` | the whole guide in one clause | prose agents | the volatile recency tail |

**Resolution is `storyline → scenario`. There is no global layer** — deliberately unlike
`promptOverrides`, because a global style guide would push one voice onto every world.
Presets are a **library, not a layer**: applying one *copies* its text into the storyline's
own fields, so nothing stores a live reference and every block stays editable afterwards.

**A scenario override is composed as an appended delta**, not substituted into the world's
guide. The storyline's guide is emitted whole, then a `FOR THIS SCENE ONLY` section carrying
just the overridden blocks and the sentence that resolves the conflict. This keeps the cached
system prefix intact up to its last bytes — an in-place rewrite would break the prefix at the
overridden block and discard everything after it. `signature` is the exception: it rides in
the volatile tail, so it resolves **in place** (scenario wins) and no delta is involved.

Write tolerance, all deliberate for an optional retrofitted feature:

- Unknown block ids are **dropped, not 422'd** (a guide from a newer build must not fail).
- A blank value at a layer means **inherit**, not "override with nothing".
- A `PATCH` replaces the whole object — it is wholesale-edited value data, like `branches`.
- Text is **canonicalised on write** (line endings, trailing whitespace, blank-line runs).
  The stored bytes *are* what the cached prefix is built from, so two saves of visually
  identical text must produce the same prompt.
