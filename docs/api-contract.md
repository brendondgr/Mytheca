...
- `POST /storylines/build` — `{ seed?, docsOverview?, storylineId?, maxCharacters?,
  ...
  ```json
  {
    ...
    "characters": [ { "name": "…", "role": "…", "traits": "…", "appearance": "…", …, "startingStats": [ { "key": "health", "value": 100 } ], "sourceDocNames": ["maerin.md"] } ],
    "settings": [ { "name": "…", "type": "…", "desc": "…", "atmosphere": "…", "features": "…", "currentState": "…", "sourceDocNames": ["chapel.md"] } ]
  }
  ```

  Each proposed character/setting carries `sourceDocNames` — the uploaded document(s)
  it was mined from (build lineage; merged when a subject spans docs). Persisted nowhere
  by the build itself; the frontend commit resolves each name to its persisted corpus
  doc id and creates a doc→entity provenance link (see Context documents above).
