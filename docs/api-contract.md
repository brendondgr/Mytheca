...
- `GET /options/scene-presets` — the named scene presets, read-only and DB-free →
  `[{ "id": "slow_burn", "label": "Slow burn", "blurb": "…",
  "values": { "maxTurns": 6, "suggestionsCount": 2, "beatLength": "long" } }]`. Four
  built-ins, authored in `content/scene_presets.py` — a preset is part of how the app reads,
  so it is code, not data an operator edits in a row (the same reasoning as the graph type
  registry). `blurb` names the **trade**, not the numbers, because the numbers are visible in
  the controls directly beneath it in the UI. Applying one is an ordinary `PATCH /scenarios/{id}`
  carrying all three values plus `scenePreset`, so there is no apply endpoint and no second
  write path. Deliberately absent from every preset: `contextBeats` (the window fits itself —
  bundling a depth would reintroduce a question only the app can answer) and any planner
  switch (that changes what the app *is*, not how a scene reads).
...
`ScenarioUpdate`'s with one deliberate difference — `maxTurns` is capped at **10** here while
the scenario column has no upper bound, because a per-turn knob that can ask for an
arbitrarily long turn is a way to hang the stream by accident. An out-of-range value is a
422 at the boundary; a hand-edited scenario row is re-clamped by the resolver
