...
| Characters | `GET /storylines/{id}/characters`, `POST /storylines/{id}/characters`, `GET /characters/{id}`, `PATCH /characters/{id}`, `DELETE /characters/{id}` | Belong to a storyline; each holds a stat block. Read/write shape: `id`, `name`, `role`, `color`, `mono` (derived), `traits`, `speech`, `goal`, `secret`, plus base-identity prose `appearance`, `background`, `personality` (all nullable), `portrait` (nullable relative `/media/...` URL of the generated WebP avatar), `portraitPositive` / `portraitNegative` (nullable ComfyUI prompt strings that produced the portrait — persisted so the author can tweak-and-re-render on re-edit), and `voiceSamples` (a list of `{ situation, sample }` pairs — the character's voice & tone profile, each pair a *full back-and-forth dialogue exchange* rather than a one-line summary; empty list when unauthored, derived from background/personality before starting stats and injected into the turn loop). |
...
- `POST /characters/voice-samples` — `{ name?, role?, traits?, speech?,
  ...
  2–3 (capped at 4) situation → dialogue-exchange pairs — from the character's prose
  ...
  (diction, rhythm, tics, formality) before writing; each `situation` is a line/beat
  that prompts the character (often another character's dialogue) and each `sample`
  is a full back-and-forth exchange (several alternating lines) in their voice, not a
  single sentence. Runs **before** starting stats (voice/tone is defined first).
