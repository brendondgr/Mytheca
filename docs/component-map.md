...
| `EntityModal` + forms | feature | `components/feature/EntityModal.tsx` with `SettingForm` / `ScenarioForm`; create/edit/delete. Now owns **Setting + Scenario only** (Character moved to `CharacterModal`, Storyline to `StorylineModal`). **Desktop (`md+`)**: two-column layout — the By-hand form fills the left, the Agentic draft panel sits in a fixed right rail, toggle hidden. **Mobile (`< md`)**: a By-hand / Agentically toggle swaps a single column. Draft is still faked for these two. |
| `CharacterModal` | feature | `components/feature/CharacterModal.tsx` — the agentic **Character Creator** (mirrors `StorylineModal`). Main column: a by-hand form (name/role/traits/voice/goal/secret + **Appearance / Background / Personality**) with an avatar+accent header; an agentic **Draft with Velora** seed (`POST /characters/draft`, optionally grounded in the world + dropped docs); a **Portrait** section (`POST /characters/portrait-prompts` → editable positive/negative → `POST /characters/portrait` renders a WebP, previewed); a **Starting stats** section (`POST /characters/starting-stats` → review/adjust → applied via `PUT /characters/{id}/stats`). A detached **context-files** column (shared `ContextFilesPanel`). Produces §1 *node properties* only — no graph. |
| `ContextFilesPanel` | feature | `components/feature/ContextFilesPanel.tsx` — shared detached context-files column used by `StorylineModal` + `CharacterModal`: drop `.txt`/`.md` (read in-browser via `lib/readDocs.ts`), per-file **Draft / RAG / KG** toggles + per-category bulk All / None + file count. Only **Draft** is wired (`docsForDraft`); RAG/KG are forward-looking seams. Files are never persisted. |
