...
| `StorylineModal` | feature | `components/feature/StorylineModal.tsx` — write-first storyline create **and edit** modal: title / genre / tagline + multi-paragraph **premise**. Create persists via `POST /storylines`; edit (prefilled, "Save Changes") via `PATCH /storylines/{id}`. Drag-and-drop context-files zone + agentic "Draft with Velora" panel are visible non-functional seams. |
