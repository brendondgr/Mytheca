...
| `StorylineModal` | feature | `components/feature/StorylineModal.tsx` — write-first storyline create **and edit** modal: title / genre / tagline + multi-paragraph **premise** + an editable agent-facing **World Primer**. Create persists via `POST /storylines`; edit (prefilled, "Save Changes") via `PATCH /storylines/{id}`. The agentic **"Draft with Velora"** seed box drafts the metadata (`POST /storylines/draft`), **"Generate primer"** writes the World Primer (`POST /storylines/primer`), and the context-files drop zone reads `.txt`/`.md` files in-browser (`lib/readDocs.ts`) to ground a single generation — files are **not** persisted (RAG deferred). |
