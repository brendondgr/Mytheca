...
| UI primitives | ui | `components/ui/` — `Monogram`, `Eyebrow`, `SectionHeader`, `Tag`, `Chip`, `ToggleChip`, `IconButton`, `Button`, `FieldLabel`, `TextField`, `TextArea`, `Modal` (portal + focus trap + Esc/backdrop; optional `externalClose` renders a reddish × hovering outside the panel, still inside the trap). |
