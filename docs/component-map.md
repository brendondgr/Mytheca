...
| `StorylineModal` | feature | `components/feature/StorylineModal.tsx` — write-first storyline create **and edit** modal. **Wide two-column head** (`md+`): a by-hand form (Title + Genre on one row, full-width Tagline, then Premise) on the left and a context/agentic rail on the right, over a **full-width World Primer row**; the **seal + color picker** sit right-justified in the header. Create persists via `POST /storylines`; edit (prefilled, "Save Changes") via `PATCH /storylines/{id}`. The agentic **"Draft with Velora"** seed box drafts the metadata (`POST /storylines/draft`), **"Generate primer"** writes the World Primer (`POST /storylines/primer`). The context-files drop zone reads `.txt`/`.md` files in-browser (`lib/readDocs.ts`); each dropped file is **showcased with per-use toggles — Draft / RAG / KG** (`ReadDoc.useDraft/useRag/useKg`). Only **Draft** is wired (it filters which files ground generation, via `docsForDraft`); **RAG/KG** are forward-looking seams (those systems deferred). Files are **not** persisted. |
