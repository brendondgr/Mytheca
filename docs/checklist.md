...
### Options menu — backend settings store + LLM proxy (Phase 1 done; plan: `docs/plans/options-menu.md`)
...
- **Remaining (Phases 2–3):** frontend header **Options ▾** dropdown + `/options` route shell (vertical tablist) + Appearance/Library/About tabs; the full **Language Models** tab (fetch/select/test/params/save). Encryption-at-rest for the key + per-owner scoping deferred to the auth phase.
