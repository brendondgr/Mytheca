...
### Options menu — settings store + LLM proxy + `/options` page (Phases 1–2 done; plan: `docs/plans/options-menu.md`)
...
- **Remaining (Phase 3):** the Language Models tab's **fetch-models dropdown** + **connection test** (wire to `POST /options/llm/{models,test}`). Live preview a11y/responsive pass was constrained by a concurrent session sharing the working dir (built in an isolated worktree); structure verified via build + tests + an earlier preview snapshot. Encryption-at-rest for the key + per-owner scoping deferred to the auth phase.
