...
- **Dev-DB gotcha (action required):** the persistent Postgres dev DB needs `ALTER TABLE storylines ADD COLUMN IF NOT EXISTS symbol TEXT DEFAULT '◆'; ALTER TABLE storylines ADD COLUMN IF NOT EXISTS symbol_color TEXT DEFAULT '#C8862A';` — idempotent `create_all` adds missing tables but does **not** ALTER existing ones. Until this runs, `GET /storylines` will error against the old table. Reinforces the standing **Alembic** follow-up.
- **Deferred:** the live in-browser a11y/responsive pass (320/375/768/1024) was blocked because applying the dev-DB ALTER is a shared-resource write outside this task's authorization; built to the established a11y idiom (`role=group`, `aria-pressed`, `aria-label` per swatch, `aria-hidden` seal glyph, `:focus-visible`) and covered by the suite. Run the pass once the ALTER is applied.
