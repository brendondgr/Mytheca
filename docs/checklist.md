...
- **Dev-DB gotcha (must apply before running the live backend on this branch):** idempotent `create_all` adds tables but not columns — run `ALTER TABLE storylines ADD COLUMN IF NOT EXISTS world_primer TEXT;` against the persistent Postgres dev DB (`backend-db-1`). Reinforces the standing **Alembic** follow-up.
