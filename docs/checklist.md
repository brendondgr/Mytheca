...
Replaces the one-shot "Untitled Storyline" create with a write-first modal. **Phase 1 (done):** added a nullable `premise` (long `Text`) column to `Storyline` (model/schema/CRUD/seed) — 41 backend tests, ruff + mypy clean. **Phase 2 (done):** the switcher's "New Storyline" now opens `StorylineModal` (title / genre / tagline + multi-paragraph **premise**); `useLibraryState.openCreateStoryline` + `submitStoryline` persist via `POST /storylines` then activate the world. 34 frontend tests green; verified end-to-end via preview (create "Tidefall" → persisted with premise via API; modal a11y: `role=dialog`/`aria-modal`/labelled, focus-trapped, Esc closes; no overflow at 320/375).
...
- **Remaining (Phase 3):** the visible non-functional **context-files drop zone** + **agentic "Draft with Velora"** seams in the modal; later, wire file ingest + agentic draft-into-fields. Mobile still can't reach "New Storyline" (switcher hidden `< md`).
