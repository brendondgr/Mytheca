...
A three-zone "open book": a **left cast rail** (At the table · turn order, portrait avatars with per-character **"Thinking"/"Speaking"** activity indicators), a **reading-first center column** (a `SceneIntro` "scene is set" band — setting, genre/tone, the player's aim, dramatis personae — then the transcript of beats, a slim **context usage bar**, and a bottom composer), and a **right director rail** (a live **"Scene pulse"** activity feed + scene-state chips). The `SceneConfigMenu` (gear **"Config"** button → popover with "Max turns" 1–10, "Suggestions" 0–4, and a "Number of beats" 5–100 slider showing a live approximate token estimate) is rendered by **`SceneHeader`** immediately **left of Export**, styled to the header-button idiom. The transcript is the primary surface and stays centered at ≤720px; the `SceneIntro` band ensures the reading column carries the full scene context even on mobile, where the rails collapse to drawers. This is deliberately **not** a generic three-pane SaaS shell or a card grid.

**Context usage bar.** A slim `role="progressbar"` bar rendered by `StoryPlayerView` in a centered `max-w-[720px]` wrapper directly above the composer, visible only when the context window was successfully fetched. Fill color is token-driven: `bg-success` (green) below 50 %, `bg-gold` at 50–75 %, `bg-danger` at or above 75 %. Hover `title` attribute and `aria-valuetext` surface `"5.2K / 16K tokens"` so the count is always available non-visually. A `width` transition animates fill changes (`motion-reduce:transition-none`).

**Flowing composer.** The composer is a single, transparent outer band wrapping a centered `max-w-[720px]` chat panel (`rounded-t-[14px]` top corners, `border-field-bd bg-field` surface, `focus-within` accent ring). An auto-growing `<textarea rows=1>` expands to fit content — height set to `scrollHeight` capped at 240 px (≈10 lines), then scrolls; the textarea always shows at least 1–2 lines and comfortably shows ≥5 lines as text grows. Right padding reserves a column for the embedded **circular Send button** — `h-[38px] w-[38px] rounded-full bg-accent`, absolutely positioned bottom-right inside the panel, centered paper-plane SVG (`aria-hidden`), `aria-label="Send"` on the button — so text wraps before reaching the button and never slides underneath it. Enter sends; Shift+Enter inserts a newline.
