...
**Graph view mode.** The `SceneHeader` carries a compact **Chat ⇄ Graph** segmented switch (left of Export). Switching to **Graph** replaces the center transcript+composer column with `GraphView` — the scenario's Story-Graph as a force-directed canvas (`react-force-graph-2d`), nodes/edges colored by type (see the Story-Graph node/edge colors bullet above) with a legend and an sr-only node/edge table. The cast + director rails stay mounted; the graph is fetched (and its renderer chunk loaded) only on first switch, and shows a calm "offline" state when the graph DB is down. Clicking a Character node opens that character's dossier.
