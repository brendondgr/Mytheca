...
**Context usage dial.** A small circular `role="progressbar"` ring (`ContextUsageDial`) in the composer's bottom controls row, immediately left of Send, visible only when the model's context-window size is known. The arc fills with `used / max` and is stroke-coloured by the same token-driven thresholds — `--color-success` (green) below 50 %, `--color-gold` at 50–75 %, `--color-danger` at or above 75 %; the centre shows the used-token count (compact `K`). It renders the **exact** `usage.prompt_tokens` the model reported for the latest turn (the true context used, not a char/4 estimate); the hover `title` + `aria-valuetext` spell out `"8.3K of 16K tokens (exact|estimated)"` so the count and its provenance are always available non-visually. The arc transition is `motion-reduce:transition-none`. It replaced the earlier slim full-width **context usage bar**.

**Two-row composer.** The composer is a transparent outer band wrapping a centered `max-w-[720px]` chat panel (`rounded-[14px]`, `border-field-bd bg-field` surface, `focus-within` accent ring) with **two rows**. **Row 1** is the auto-growing `<textarea rows=1>` (full width; expands to `scrollHeight` capped at 240 px ≈10 lines, then scrolls). **Row 2**, separated by a `border-t`, is the controls bar: **Config** on the bottom-left (its popover opens **upward** — `openUp` — so it clears the screen edge; the "Number of beats" readout reflects the **real** recent transcript when `beatTexts` is supplied), a flexible blank gap reserved for future options, then on the right the **context dial** and a **"Send →" pill** (`bg-accent`, `Send` label + right-arrow SVG, `aria-label="Send"`). Enter sends; Shift+Enter inserts a newline; the textarea stays editable while a turn streams (only Send + Enter are blocked via `sendDisabled`).
