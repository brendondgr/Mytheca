...
- **Qdrant point ids.** Qdrant requires uint/UUID point ids; Mytheca ids are prefixed strings. Map every entry id → `uuid5(NAMESPACE, entry_id)` deterministically and keep the original id in the payload.
...
- **One collection, storyline-filtered.** A single `mytheca_lore` collection; every point carries `storyline_id` in its payload and retrieval filters on it, so a storyline only ever retrieves its own corpus.
...
- **Action:** Run `uv run pytest utils/tests/backend/rag`. Once green, commit: `Mytheca RAG (1/8) Complete: front-matter schema + prefix-fusion serializer + entity entry adapters.`
...
- **Action:** Run `uv run pytest utils/tests/backend/rag`. Once green, commit: `Mytheca RAG (3/8) Complete: Qdrant container + best-effort client + dense/sparse collection store.`
...
- **Action:** Run `uv run pytest utils/tests/backend`. Once green, commit: `Mytheca RAG (4/8) Complete: embed-on-save with live progress + delete-from-index hooks in CRUD.`
...
- **Action:** Run `uv run pytest utils/tests/backend`. Once green, commit: `Mytheca RAG (5/8) Complete: hybrid retrieval (dense+BM25+RRF+pre-filter) wired into the authoring agents.`
...
- **Action:** Run `uv run pytest utils/tests/backend` + `npm test`/`typecheck` + a11y/responsive pass. Once green, commit: `Mytheca RAG (6/8) Complete: entity-scoped persistent context docs across all four menus + storyline edit-reload fix.`
...
- **Action:** Run `uv run pytest utils/tests/backend` + `npm test`. Once green, commit: `Mytheca RAG (7/8) Complete: delete-from-index across all delete paths + RAG status/re-embed surface.`
...
- **Action:** Full `uv run pytest` + `npm test` + `typecheck`/`build` + final a11y/responsive pass. Once green, commit: `Mytheca RAG (8/8) Complete: docs + full validation.` Then merge to `main`.
