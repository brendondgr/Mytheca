...
| `/play/[scenarioId]` | **Story player** — live, streamed scene | User | Postgres + Redis + event stream | `features/story-player` + `components/feature` (narrator cards, side panels) | `POST /play/{scenarioId}/turn`, `GET /stream/{sessionId}` (SSE/WS) | loading, empty, **streaming/partial**, stalled-stream, reconnect, error, permission, mobile, reduced-motion |
