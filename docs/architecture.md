...
- **Vector DB** (deferred) — semantic memory retrieval and the graph's vector entry-point (§7.1); build the memory seam behind an interface so it can be added without rewrites.
