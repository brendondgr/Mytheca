...
**Frontend tests are co-located** (`Foo.tsx` → `Foo.test.tsx`) — 151 files, 1543 cases. `utils/tests/frontend/` is empty; ignore it.

## Backend tests — `utils/tests/backend/`

Five area folders: `api/` `agents/` `services/` `rag/` `data/`, plus a shared `conftest.py`. Add new tests to the matching folder as `test_<behavior>.py`. 2195 cases pass today.
