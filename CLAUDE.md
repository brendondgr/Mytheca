...
| Turn loop / orchestration | `services/` | `turn_engine.py` (POV loop+delta stream) `assembler.py` (context) `retrieval_gate.py` `emission.py` `validator.py` (stat clamp) `events_store.py` `turn_writer.py` `crud.py` `consistency.py` `reflection.py` `relationships.py` `type_registry.py` `stat_guidance.py` `settings_store.py` `llm.py`/`llm_backend.py` `media.py`/`media_cleanup.py`/`portraits.py`/`scene_art.py`/`comfyui.py` `graph_reader.py`/`graph_writer.py` `concurrency.py` |
